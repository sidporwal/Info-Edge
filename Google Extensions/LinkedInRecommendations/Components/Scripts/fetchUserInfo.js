fetchUserContactInfo = () => {
  const contactLinks = document.getElementsByClassName(
    "pv-contact-info__contact-link"
  );
  const birthDate = document.getElementsByClassName(
    "pv-contact-info__contact-item"
  );
  const contactNo = document.getElementsByClassName(
    "pv-contact-info__contact-type ci-phone"
  );
  const data = {
    userProfileUrl: contactLinks[0].innerText,
    address: contactLinks[1] && contactLinks[1].innerText,
    mail: contactLinks[2] && contactLinks[2].innerText,
    dob: birthDate[0] && birthDate[0].innerText,
    contactNo:
      contactNo[0] &&
      contactNo[0].children[2] &&
      contactNo[0].children[2].innerText,
  };
  console.log(data);
};
...
const fetchUserInfo = () => {
  ...
  setTimeout(() => {
    document
      .getElementsByClassName("ember-view link-without-hover-state")[0]
      .click();
